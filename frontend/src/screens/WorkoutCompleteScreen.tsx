...
import { ExerciseLog, RootStackParamList, WorkoutRecord } from '../types';
  ...
  return {
    ...
    exercises: logs
      .map(log => ({
        ...
        sets: log.sets.filter(s => s.weight > 0),
      }))
...
export default function WorkoutCompleteScreen({ route, navigation }: Props) {
  const { logs, program, durationSeconds } = route.params;
  const { bottom } = useSafeAreaInsets();

  const record = buildRecord(logs, program, durationSeconds);
